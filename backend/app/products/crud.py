...
class ProductsSet:
    ...
    def _filter_products_stmt(
        ...
        if owner:
            if only_owner:
                ...
                stmt = stmt.where(or_(Product.owner_id == owner.id, Product.owner_id == None))
        else:
    ...
    async def _get_products_count(
        self, ids: list[int] | None = None, owner: DbUser | None = None, only_owner: bool = False
    ) -> int:
        count_stmt = select(func.count()).select_from(Product)
        filtered_count_stmt = self._filter_products_stmt(count_stmt, ids, owner, only_owner)
    ...
    async def _get_paginated_products(
        ...
        filtered_result_stmt = self._filter_products_stmt(result_stmt, ids, owner, only_owner)
    ...
    async def all(
        ...
        products_count = await self._get_products_count(ids, owner, only_owner)
        pages_count = math.ceil(products_count / per_page)
        page, per_page = self._validate_page_per_page(page, per_page, pages_count)
        offset = (page - 1) * per_page
        result_products = await self._get_paginated_products(offset, per_page, ids, owner, only_owner)
